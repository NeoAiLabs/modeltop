...
class _DrafterTransport(httpx.AsyncBaseTransport):
    def __init__(self, streams: list[httpx.AsyncByteStream] | None = None) -> None:
    ...
    async def scenario() -> None:
        ...
        async with app.run_test(size=(120, 36)) as pilot:
            ...
            rendered = "\n".join(
                ...
                for widget in view.query_one(DrafterResultsPanel).query(Static)
            )
            assert "ACCEPT" in rendered.upper() or "0.67" in rendered
            assert "DRAFTER" in _plain_render(app.query_one("#header-subtitle", Static))
            footer = _plain_render(app.query_one(StatusFooter))
            assert "accept" in footer.lower() or "0.67" in footer or "DRAFTER" in footer
